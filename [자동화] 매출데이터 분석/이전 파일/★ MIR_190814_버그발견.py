...
# 맵핑 엑셀파일 불러오기
map_df = pd.read_excel('D:/0. 원단위/과목-CP사 맵핑-190528.xlsx', sheet_name='mapping Data')
...
# 매핑을 위해 데이터 정리
df = df.replace('과목:','')
df = df.rename(columns={'과목명(품목)': '맵핑용 과목명'})
map_df = map_df.rename(columns={'과정명': '맵핑용 과목명'})

## 카테고리 및 CP사 맵핑하기, 데이터 병합 시 카테고리 있는 것만 ㅠ.ㅠ 당겨옴
results = df.merge(map_df, on='맵핑용 과목명')
...
# 저장위치 설정
save_path = '//172.20.83.178/컨텐츠매니지먼트팀/19. MIR/2019년/'

# raw 데이터 결과 값 저장하기

writer = pd.ExcelWriter(save_path + 'results_' + today + '.xlsx')
